<template>
    <div v-if="!isOpen">
    <div class="video-preview" v-if="previewImg">
        <img :src="previewImg" @click="this.isOpen = !this.isOpen;"></div>
...
export default {
  ...
  mounted() {
    ...
    this.currentUrl = this.playlist[currentIndex];
  },    
  methods: {
    generatePlaylist() {
        return _.filter(this.jobs, function(job) { return job.status == 'finished' && job.url != ""; });
    },
    close() {
