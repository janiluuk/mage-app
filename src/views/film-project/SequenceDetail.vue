...
const sequence = computed(() => store.getters['FilmProject/sequence']);
const shots = computed(() => 
  store.getters['FilmProject/shotsBySequence'](sequenceId.value)
);
...
const saveShot = async () => {
  ...
  try {
    if (editingShotId.value) {
      ...
      await store.dispatch('FilmProject/' + actions.CREATE_SHOT, {
        projectId: projectId.value,
        sequenceId: sequenceId.value,
        data: shotForm.value,
      });
...
const deleteShot = async () => {
  ...
  try {
    await store.dispatch('FilmProject/' + actions.DELETE_SHOT, {
      projectId: projectId.value,
      sequenceId: sequenceId.value,
