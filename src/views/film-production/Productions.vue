  ...
  <div class="productions-page">
    <div class="card">
      <div class="flex justify-content-between align-items-center mb-4">
        <div>
          <h2 class="text-3xl font-bold m-0">Film Productions</h2>
        ...
        <template #header>
          <div class="flex justify-content-between align-items-center">
            <span class="p-input-icon-left">
              ...
              <InputText v-model="filters['global'].value" placeholder="Search productions..." />
            </span>
          </div>
        </template>
          ...
          <template #body="{ data }">
            <div class="flex gap-2">
              <Button 
                ...
                v-tooltip.top="'View Production'"
                @click="viewProduction(data.id)"
              />
              <Button 
                icon="pi pi-pencil" 
                class="p-button-rounded p-button-text"
                v-tooltip.top="'Edit Production'"
                @click="editProduction(data)"
              />
              <Button 
                icon="pi pi-trash" 
                class="p-button-rounded p-button-text p-button-danger"
                v-tooltip.top="'Delete Production'"
    ...
    >
      <div class="flex flex-column gap-3">
        <div class="field">
          <label for="name" class="font-semibold block mb-2">Production Name *</label>
          <InputText 
            id="name" 
    ...
    >
      <div class="flex align-items-center gap-3">
        ...
        <span>Are you sure you want to delete "{{ productionToDelete?.name }}"? This action cannot be undone.</span>
      </div>
      <template #footer>
        <Button 
          label="Cancel" 
...
const productions = computed(() => store.getters['FilmProduction/productions']);
...
const dialogTitle = computed(() => editingId.value ? 'Edit Production' : 'New Production');
...
const saveProduction = async () => {
  ...
  try {
    if (editingId.value) {
      ...
      await store.dispatch('FilmProduction/' + actions.CREATE_PRODUCTION, formData.value);
    }
...
const deleteProduction = async () => {
  ...
  deleting.value = true;
  try {
    await store.dispatch('FilmProduction/' + actions.DELETE_PRODUCTION, productionToDelete.value.id);
...
const loadProductions = async () => {
  try {
    await store.dispatch('FilmProduction/' + actions.GET_PRODUCTIONS);
  } catch (error) {
